...
const explosionConfig = {
  ...
  /* ========= 物理效果 ========= */
  gravity: 0.15,       // 重力
  initialVelocityY: -4, // 向上弹射力度
  initialVelocityX: 0.8, // 横向扩散范围
  lifeDecay: 0.02,     // 消失速度
  
  /* ========= 颜色设置 ========= */
  colors: [
    "#FFD700", "#FF6347", "#00BFFF", "#32CD32", "#FF69B4", 
    "#8A2BE2", "#FFA500", "#00FA9A", "#FF00FF"
  ],
...
const getRandomColor = () => {
  return explosionConfig.colors[Math.floor(Math.random() * explosionConfig.colors.length)];
};
...
class CursorExplosionEffect {
  ...
  createParticle(pos, text, color, scale) {
    ...
    this.particles.push({
      ...
      color: color,
      scale: scale,
      vx: getRandomRange(-explosionConfig.initialVelocityX, explosionConfig.initialVelocityX),
...
try {
  ...
  console.log("Multi-Cursor Explosion Effect Loaded.");
} catch (e) {
  console.error("Effect Load Failed:", e);
}